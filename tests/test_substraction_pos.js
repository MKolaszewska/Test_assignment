...
describe('Create Android session', async function () {
  ...
  it('substraction positive test', async function () {
    const res = await client.status();

    const digit1 = await client.$('~9');
    const digit5 = await client.$('~4');
    const plus = await client.$('~minus');
    await digit1.click();
    await plus.click();
    await digit5.click();
